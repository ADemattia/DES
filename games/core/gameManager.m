classdef gameManager < handle    
    ...
    properties
        ...
        trajectoryId % array storing the history of activated ids
    ...
    methods
        ...
        function updateProfile(obj, managerId, action, externalClock)
            ...
            obj.trajectoryId{end+1} = managerId; 
        ...
        function displayStats(obj, adjacencyMatrix)
            ...
            hold(ax, 'on');
            highlight = plot(ax, NaN, NaN, 'o', ...
                'MarkerSize', 20, 'MarkerEdgeColor', [1 0.85 0], ...
                 'LineWidth', 3, 'HandleVisibility', 'off');
 
            updateFrame = @(step) obj.renderStep(step, ax, p, cmap, uniqueActions, fixedX, fixedY, highlight);
        ...
        function renderStep(obj, step, ax, plotHandle, cmap, uniqueActions, x, y, highlight)
            % renderStep: updates the network graph for the given trajectory step
            step = round(step);
            ...
            % map each player's action to the corresponding color index
            colorIndices = arrayfun(@(val) find(uniqueActions == val, 1), currentProfile);
            plotHandle.NodeColor = cmap(colorIndices, :);
            plotHandle.XData = x;
            plotHandle.YData = y;
        
            % highlight the activated node
            activeNode = obj.trajectoryId{step};
            if activeNode > 0
                highlight.XData = x(activeNode);
                highlight.YData = y(activeNode);
            else
                % initial state: no activation has occurred yet
                highlight.XData = NaN;
                highlight.YData = NaN;
            end
